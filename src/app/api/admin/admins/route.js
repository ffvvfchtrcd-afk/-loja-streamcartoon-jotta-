...
export async function POST(request) {
  ...
  if (!username || !password) return NextResponse.json({ error: 'Dados inválidos' }, { status: 400 })

  const existing = await prisma.admin.findUnique({ where: { username } })
  if (existing) return NextResponse.json({ error: 'Username já existe' }, { status: 400 })
